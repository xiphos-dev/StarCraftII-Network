...
# dtype = [('estructura', 'S25'), ('tiempo', int)]
# fila=[[str(999), 999] for _,_ in enumerate(range(999,1025))]
# fila = sorted(fila, key=lambda x: x[1], reverse=True)
# print([x[1] for x in fila])
# #output = []
# #output.append(fila[:])
...
#x_batch_rnn, x_batch_rnn_unidades, x_batch_rnn_mejoras,x_batch_rnn_mid, x_batch_rnn_unidades_mid, x_batch_rnn_mejoras_mid, y_mid, vacio_mapa, vocabulario_mid, vocabulario_unidades_mid, vocabulario_mejoras_mid = inputRecurrenteConIntervalo(X,y2,False,False,0,700)
...
_,x_batch_rnn, y_mid, vacio_mapa, vocabulario = inputRecurrenteUnico(X,y2,False,False,700)
